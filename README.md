...
| 1 | PHP core concepts |
| 2 | MySQL |
| 3 | WordPress core — hooks, `functions.php`, menus, enqueue, widgets, child themes, template hierarchy |
| 4 | Plugin development — structure, REST API, shortcodes, settings pages, security, custom tables, cron jobs |
| 5 | REST API and JWT authentication — what it is, how it works, why it is secure |
| 6 | Advanced Custom Fields — Repeater vs Flexible Content, the `acf/save_post` hook |
| 7 | Directory websites — custom post types, taxonomies and `meta_query` working together |
| 8 | WooCommerce — price, cart, checkout, order status and thank-you hooks, definition and implementation |
| 9 | SEO and performance — Core Web Vitals, caching, image optimisation |
| 10 | Debugging — `WP_DEBUG`, common errors, isolating a plugin conflict |

Elementor and the Figma to WordPress workflow run through the later chapters: the pixel-perfect build process, custom widgets, and the performance problems page builders bring with them.
...
Building WordPress plugins too? The custom plugins referenced throughout this guide live at **[dev-noman501/my-plugins](https://github.com/dev-noman501/my-plugins)**.
